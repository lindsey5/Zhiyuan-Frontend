...
export default function SponsoredItemDetails({
  close,
  sponsoredId,
}: SponsoredItemProps) {
    const { getSponsoredItemById, updateSponsoredItemStatus } = useSponsoredItem();
    const { data } = getSponsoredItemById(sponsoredId || "undefined");
    ...
    const handleUpdate = async (status: string) => {
        ...
        const data = await promiseToast(updateSponsoredItemStatus.mutateAsync({
            id: sponsoredId,
            status
        }), "top-center", () => {});
